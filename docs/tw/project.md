...
胡桃-Remastered是一款基於 MIT 協議開源的原神工具箱，是依託原胡桃工具箱項目開發的，專為現代化 Windows 平臺設計，旨在改善桌面端玩家的遊戲體驗。通過結合官方資源與團隊自主設計的全新功能，它提供了一套全面而實用的工具集，無需依賴任何移動設備。同時，它嚴格遵守非破壞性原則，不對遊戲客戶端進行任何修改，確保工具箱的使用安全性。

Snap Hutao-Remastered 項目採用 MSI 格式進行打包。
...
但是沙箱環境中具有無法規避的限制導致部分功能無法正常使用，為了安裝和使用體驗，胡桃-Remastered 採用了 MSI 格式以在這樣的特殊運行環境中規避顯然是合理的。同時代碼內部增強後同時提供了對 MSIX 環境的支持，用戶可自行編譯並打包使用。
