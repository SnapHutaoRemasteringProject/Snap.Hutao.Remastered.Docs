...
---
::: caution
开发人员不为任何功能负责，使用相关功能即视为用户同意自行承担相关风险。
:::
# 高级启动器
...
自定义启动可以让你便捷地一键运行胡桃工具箱外部的程序。
- 启动自定义程序
  - 可以快捷在胡桃内启动指定路径的应用程序，同时你可以下载自定义 json 文件来扩展更多的启动选项。请确保 json 文件的来源和清单内容可靠，以免造成不必要的损失。
  - 清单数据位于 settings.json 文件中，位于 `%USERPROFILE%\AppData\Local\Hutao\settings.json` 路径下。
- 启动延迟启动清单中的程序
  - 你可以设置延迟启动清单列表中的程序，左键点击后将同时执行列表中的程序开始计时并按时执行，并非存在顺序。如果以非管理员账号运行胡桃工具箱，则清单中的程序也可能将以非管理员权限启动，在遇到强制需要 UAC 权限的程序时，UAC 弹窗可能会造成延迟时间不准确。
  - 清单数据将保存在 userdate.db 数据库中。
- 连带启动
  - 你可以选择在胡桃内启动游戏时和启动自定义程序时是否同时触发延迟启动清单中的程序
- 自定义启动暂不支持传入启动参数
...
1. **定位备份文件**：
   ...
   - 针对新版胡桃工具箱Remastered，前往以下路径：`%USERPROFILE%\AppData\Local\Hutao`。如果你手动查找那么 AppData 文件夹默认是隐藏的，若无法找到，请确保已启用显示隐藏文件夹的选项。
